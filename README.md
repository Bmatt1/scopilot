...
| `ADMIN_PASSWORD` | No | Optional URL-key fallback for `/admin?key=…` (useful for curl/scripts). The preferred admin auth path is logging in as a contractor with `is_admin = true` on their row — see `docs/ACCESS.md`. |
